...
**`review-panel` / `history-panel`** — revisão datada + atividade recente (spec V4 §§17–18): counters Consolidado/Reforço/Não-comprovado resolvem labels via criterion/project lookup mesmo sem evento recente; até 3 atalhos ⚠ Reforçar alimentam o filtro ⌘K (`data-goto-filter`, valor fatiado em 40 chars); bloco de atalho omitido quando vazio. `export-panel` / `readiness-panel` seguem stubs V4.4–V4.5. comprovado% do dashboard é fração de critérios `proven` (+ projetos contam em seus próprios cards); após migração pode cair até o usuário marcar ✅ — comportamento esperado, não bug.
